    ...
    with torch.no_grad():
        ...
        for i in range(max_output_length):
            ...
            if predicted_token == am_stoi[eos_token]:
                break
        translation = [am_itos[idx] for idx in trg_ids]
    return translation
...
def translate():
    ...
    translation = translate_sentence(sentence, model, ge_stoi, ge_itos, am_stoi, am_itos, sos_token, eos_token, device)
    return jsonify({'translation': ' '.join(translation)})

# Run the app
if __name__ == '__main__':
    # Run the app on port 8080
    app.run(host='0.0.0.0', port=100000, debug=True)
